...
// LoginWithCookies injects raw browser cookies and verifies the session.
// cookieStr is the browser's cookie header string, e.g.
// "cf_clearance=xxxx; JSESSIONID=yyyy; ..."
func (c *Client) LoginWithCookies(cookieStr string) (err error) {
	color.Cyan("Verifying session with provided cookies...\n")

	jar, _ := cookiejar.New(nil)
	cfURL, _ := url.Parse(c.host)
	...
	handle, err := findHandle(body)
	if err != nil {
		return errors.New("Cookie verification failed: not logged in. Make sure you copied the cookies while logged in on codeforces.com")
	}
...
// ConfigLoginWithCookies guides the user to paste browser cookies for login.
func (c *Client) ConfigLoginWithCookies() (err error) {
	if c.Handle != "" {
		color.Green("Current user: %v", c.Handle)
	}
	color.Cyan("Login with browser cookies (for Cloudflare-protected access)")
	color.Cyan("Steps:")
	color.Cyan("  1. Open https://codeforces.com in your browser and log in manually.")
	color.Cyan("  2. Open DevTools (F12) -> Application -> Cookies -> https://codeforces.com")
	color.Cyan("  3. Collect all Name=Value pairs, join with '; ' into one line.")
	color.Cyan("     Tip: extensions like 'Cookie-Editor' can export a header string directly.")
	fmt.Printf("Paste cookie string: ")
	cookieStr := util.ScanlineTrim()
	if cookieStr == "" {
		return errors.New("No cookies provided")
	}
	return c.LoginWithCookies(cookieStr)
}
